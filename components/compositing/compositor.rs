...
use webrender;
use webrender_traits::{self, ScrollEventPhase, ScrollLocation};
...
impl<Window: WindowMethods> IOCompositor<Window> {
    ...
    fn handle_window_message(&mut self, event: WindowEvent) {
        match event {
            ...
            WindowEvent::Scroll(scroll_location, cursor, phase) => {
                match phase {
                            TouchEventType::Move => self.on_scroll_window_event(scroll_location, cursor),
                            TouchEventType::Up | TouchEventType::Cancel => {
                                self.on_scroll_end_window_event(scroll_location, cursor);
                            }
                            TouchEventType::Down => {
                                self.on_scroll_start_window_event(scroll_location, cursor);
                            }
                }
    ...
    fn on_touch_move(&mut self, identifier: TouchId, point: TypedPoint2D<f32, DevicePixel>) {
        match self.touch_handler.on_touch_move(identifier, point) {
            TouchAction::Scroll(delta) => {
                match point.cast() {
                    Some(point) => self.on_scroll_window_event(ScrollLocation::Delta(delta.to_untyped()), point),
        ...
        match self.webrender_api {
            Some(ref webrender_api) => {
                ...
                if let Some(combined_event) = last_combined_event {
                    ...
                    webrender_api.scroll(scroll_location, cursor, combined_event.phase);
            ...
            None => {
                for event in std_mem::replace(&mut self.pending_scroll_zoom_events,
                                                     Vec::new()) {
                    if let ScrollLocation::Delta(delta) = event.scroll_location {
                        let calculated_delta = TypedPoint2D::from_untyped(&delta) / self.scene.scale;
                        let cursor = event.cursor.to_f32() / self.scene.scale;

                        if let Some(ref mut layer) = self.scene.root {
                                    layer.handle_scroll_event(calculated_delta, cursor);
                        }
                        ...
                        // TODO: Unimplemented, home/end scrolling in non-webrender.
                    }
